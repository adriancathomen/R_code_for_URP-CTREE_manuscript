...
#### >>>> Subsetting ####
names(data_EMSCI)
data.complete <- data_EMSCI[,c(1,3,6,7,9,11,12,13,14,15,17,18,24,31,32,26)]
names(data.complete) <- c("ID", "Sex", "Age", "Cause", "ExamStage", "AIS", "NLI", "PT", "UEMS",
                          "LEMS", "PP", "LT", "SCIM_Mob","speed6min", "speed10m", "min6")

# reshape to wide format
data.complete_wide <- reshape(data.complete, v.names = c("AIS", "NLI", "PT", "UEMS", 
...
m.complete_wide <- data.complete_wide[,c("ID", "Sex", "Age", "Cause", "AIS.1","NLI.1","PT.1","UEMS.1","LEMS.1","PP.1","LT.1", "SCIM_Mob.1", "speed6min.1", "speed10m.1", "min6.1", 
...
names(m.complete_wide) <- c("ID", "Sex", "Age", "Cause", "AIS_va", "NLI_va", "PT_va", "UEMS_va", "LEMS_va", "PP_va", "LT_va", "SCIM_va","speed6min_va", "speed10m_va", "min6_va",
                            "AIS_1month", "UEMS_1month", "LEMS_1month", "PP_1month", "LT_1month", "SCIM_1month","speed6min_1month", "speed10m_1month", "min6_1month",
                            "AIS_3months", "UEMS_3months", "LEMS_3months", "PP_3months", "LT_3months", "SCIM_3months","speed6min_3months", "speed10m_3months", 
                            "min6_3months", "AIS_6months", "UEMS_6months","LEMS_6months", "SCIM_6months","speed6min_6months", "speed10m_6months", "min6_6months")

m.tetra.complete_wide <- subset(m.complete_wide, PT_va == "Tetra" & !is.na(PT_va))
...
tree_tetra_va.6_6MWT<-ctree(min6_6months ~ Age + NLI_va + LEMS_va + UEMS_va + 
                                  LT_va + PP_va , 
